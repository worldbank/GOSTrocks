...
def calculate_think_hazard_score(inD, raster_path, depth_threshold, idx_col):
    ...
    Returns:
        DataFrame with columns: idx_col, mean_val, frac_area_flooded
        - mean_val: mean depth of all wet pixels (depth > 0), matching TH_FL_utils logic
        - frac_area_flooded: % of valid land pixels with depth > depth_threshold
          Valid pixels exclude nodata (negative values / raster nodata flag).
    """
    with rasterio.Env(GDAL_HTTP_UNSAFESSL='YES'):
        curRaster = rasterio.open(raster_path)
        # Read nodata value from raster metadata; fall back to Fathom default
        nodata_value = curRaster.nodata
        ...
        for idx, row in inD.iterrows():
            ...
            try:
                ...
                # rasterize the geometry (all_touched=True matches TH_FL_utils behaviour)
                geom_mask = rasterize(
                    [(geometry, 0)],
                ...
                # Nodata mask: flag the raster nodata value AND any remaining negatives
                nodata_mask = (data == nodata_value) | (data < 0)

                # Combined mask: outside geometry OR nodata pixel
                combined_mask = geom_mask.astype(bool) | nodata_mask

                # Extract only valid land pixels
                valid_values = data[~combined_mask]

                if len(valid_values) > 0:
                    # Mean of all wet pixels (depth > 0), consistent with TH_FL_utils
